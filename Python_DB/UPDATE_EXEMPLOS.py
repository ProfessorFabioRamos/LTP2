...
""")
def inserir_pedidos(cliente_id,valor,status):
...
#cursor.execute("UPDATE pedidos SET status = 'concluido' WHERE id = 1") # ATUALIZAR UM CAMPO PELO ID
...
cursor.execute("UPDATE pedidos SET status = 'concluido' WHERE cliente_id = 22") 
conexao.commit()
conexao.close()